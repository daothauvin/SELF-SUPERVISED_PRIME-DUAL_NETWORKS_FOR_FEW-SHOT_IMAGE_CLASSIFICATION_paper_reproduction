...
class Backbone(nn.Module):
  def __init__(self, type = 18):
    ...
    if type == 18:
    	...
    	resnet = get_model("resnet10",pretrained = True)
...
class GNN(nn.Module):
  def __init__(self, input_features, nf, n_way, n_support,w_init_eye=True):
    """
      K: number of classes
  ...
  def forward(self, data):
    ...
    '''
    X = data
    W_init = torch.eye(self.K*(self.N + 1)).to(data.get_device() if data.get_device() >= 0 else "cpu")
    if not self.w_init_eye:
      for i in range(0,self.N,1):
        W_init[(self.N+1)*i:(self.N+1)*i + self.N,(self.N+1)*i:(self.N+1)*i + self.N] = 1
      W_init[self.N::self.N+1] = torch.rand((self.K,W_init.shape[1]))
    if torch.cuda.is_available():
      W_init = W_init.cuda()
...
class Model(nn.Module):
  ...
  def forward_dual(self, x): # forward prime et dual sont les m̻mes fonctions en inversant prime et dual, une fonction suffit
       # first apply Resnet on x 
      if torch.cuda.is_available():
        x = x.cuda() 
      ...
      for i in range(self.n_way):  
        ...
        dual_in[:, self.n_support-1+(self.n_support+1)*i:self.n_support+(self.n_support+1)*i,:128] = prime_in[:,self.n_support+(self.n_support+1)*i:self.n_support+1+(self.n_support+1)*i,:128]
        dual_in[:, self.n_support+(self.n_support+1)*i:self.n_support+1+(self.n_support+1)*i,:128] = prime_in[:,self.n_support-1+(self.n_support+1)*i:self.n_support+(self.n_support+1)*i,:128]
...
def train(model, train_loader, test_loader, path,   nb_epoch, lr, alpha = 0.5, mode = "Prime"):
  if mode == "Prime":
    optimizer = torch.optim.Adam(model.prime.parameters(), lr=lr)
  elif mode == "Dual":
    optimizer = torch.optim.Adam(model.dual.parameters(), lr=lr)
  optimizer_backbone = torch.optim.Adam(model.fc.parameters(), lr=lr)
  model.train()
  max_acc = 0
